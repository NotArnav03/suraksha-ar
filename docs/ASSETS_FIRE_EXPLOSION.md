...
| `ext_co2.glb` | **matte black** | **huge black conical horn**, the horn is the tell, 0.25m long | `CO₂` |
...
### 3. Keep them realistic but light: the rules
...
- Scale in **metres**: ext `0.55m tall`, panel `1.0m tall`, measured with `N` panel → `Dimensions`.
...
Keep the `labelTexture` sprite: real geometry + canvas label reads in AR. `fire_panel` is **not** loaded this way; create `Points` fire there.
