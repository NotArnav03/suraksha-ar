...
wrapper, and that choice isn't cosmetic. WebXR `immersive-ar` only works in real
...
needs a real, live HTTPS URL to wrap**, not a local file build. That's
...
`android/` is gitignored, because it's fully regenerated from `twa-manifest.json`
...
the wrong place, because GitHub Pages project sites can't serve anything at the
...
- `GradleWrapper` invokes a bare `gradlew.bat` (no path qualifier), and that
...
- `JdkHelper.runJava` (used for `apksigner`) invokes an absolute path to
  `java.exe`, and anything under `Program Files` contains a space, so `cmd.exe`
  splits it at the space and reports `'C:\Program' is not recognized`.
  Quoting the path when it contains a space fixes it.

Both live in `node_modules`, so they don't survive `npm install`, which is
